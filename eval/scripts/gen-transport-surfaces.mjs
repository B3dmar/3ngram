...
const surfaces = {
  _meta: {
    ...
    captureMethod:
      'MCP surface: the REAL tools/list + prompts/list payload the MCP SDK emits, captured by wiring the SDK in-memory transport between a Client and the app McpServer (apps/server createMcpServer over the live TOOLS registry) and calling listTools()/listPrompts(). The SDK does its own Zod->JSON-Schema serialization; this is the exact payload a host injects, verifiable against the MCP Inspector. CLI: USAGE banner + per-command arg surface from apps/cli. REST: route contracts from apps/server/src/rest/router.ts; each request schema is the schema the route ACTUALLY .parse()s, serialized through the SAME SDK path as the MCP surface. search is serialized from searchQuerySchema (query+limit+type/scope/project/status filters+asOf), the schema the route parses (the MCP search tool registers the same wider schema); the other routes parse the same shapes their MCP tools register.',
    generatedFrom:
