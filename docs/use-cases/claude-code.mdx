...
<Steps>
  ...
  <Step title="Wire the hook events">
    ...
    <Note>
      ...
      `hookSpecificOutput.additionalContext` JSON payload, which Claude Code injects
      into the model's context before the edit runs (plain `PreToolUse` stdout, by
      contrast, only reaches the hook transcript — view with `Ctrl-R`). So the
      surfaced memories do reach the model, the same way the `SessionStart` briefing
      does. It stays fire-and-forget: any failure exits silently and never blocks the
      edit.
    </Note>
