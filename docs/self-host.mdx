...
- Node.js 22 or later and pnpm 11.9.0 — the repo pins `pnpm@11.9.0` via the `packageManager` field, so run `corepack enable` to use the pinned version (Node ≥ 25 no longer bundles corepack; if `corepack enable` fails with "command not found", install pnpm directly with `npm install -g pnpm@11.9.0`)
...
<Steps>
  ...
  <Step title="Seed data and mint an API key">
    ...
    # corepack not available (Node ≥ 25 dropped it)? use instead:
...
- **Worker container**: both Compose stacks include a `worker` service (built from `apps/worker/Dockerfile`) running the background consolidation, surfacing, and OAuth-client GC jobs alongside the server and data plane. The session closer additionally requires `SESSION_CLOSER_ENABLED=true` (default `false`) **and** the `LLM_GATEWAY_URL` + `LLM_GATEWAY_API_KEY` pair — with the flag on but no gateway, every closer pass skips as `no-gateway` and resolves nothing; see [Container image](/container-image). Overdue commitments stay open for `COMMITMENT_EXPIRY_GRACE_DAYS` (default 14) before that sweep expires them.
- **Seeded embeddings are pre-computed**: the seed loads committed cached vectors (`eval/fixtures/embeddings-openai-large-1536.json`) into the seeded memories, so no external API call is needed to embed the seed data itself. The search endpoint (`POST /api/v1/search`) still requires an embedding provider (`LLM_GATEWAY_URL` + `LLM_GATEWAY_API_KEY`) to embed the **query** — without one it returns `503 embedding_unavailable`. Writes do not require a provider: without one, new memories are still accepted (`201`) with embedding deferred (`"embedded":"off"`).
...
- **Search returns `503 {"error":"embedding_unavailable"}`** — embeddings are off; writes still succeed (`201`, `"embedded":"off"`) without a provider, but search needs one to embed the query. Set `LLM_GATEWAY_URL` + `LLM_GATEWAY_API_KEY` to enable vector search.
