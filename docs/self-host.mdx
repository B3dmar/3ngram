...
- **Worker container**: both Compose stacks include a `worker` service (built from `apps/worker/Dockerfile`) running the background consolidation, surfacing, and OAuth-client GC jobs alongside the server and data plane. The session closer additionally requires `SESSION_CLOSER_ENABLED=true` (default `false`); see [Container image](/container-image).
