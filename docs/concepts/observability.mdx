...
- `Mcp-Method`/`Mcp-Name` are observed before body parsing through bounded allowlists. Unknown values collapse to sentinel labels, so attacker-controlled headers cannot create metric cardinality or enter logs. This signal includes rejected requests; the post-validation tool-call counter remains the validated-dispatch source.
