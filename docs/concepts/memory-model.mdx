...
A superseded memory is marked invalid from that point — not deleted, not rewritten. Retrieval is supersession-aware: ranked `search` **demotes** superseded predecessors (it does not drop them); chronological listing, briefing, and the dashboard list show only currently-valid rows (`status = active` AND `valid_to IS NULL`). `asOf` (`validAt` and/or `asKnownAt`) is time travel, so "what did I believe on date X" stays answerable.
