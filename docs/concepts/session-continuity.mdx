...
tool-I/O). Its honest scope is **commitment-resolution and this-run re-checks** against
briefed ids, this-run provenance events, and a bounded last-message excerpt. It cannot
reconstruct a decision, preference, or user-turn remark that nobody recorded — that
hole is the same structural miss `/debrief` already has (user-turn feedback never
appears in tool I/O). Named here the way the stale-gate hole is named, not papered
over. Compatible with 2026-06-11: intentional context, not a scrape.
...
agent_sessions (
  ...
  activation_epoch     int not null,         -- incremented on startup/resume; close must match
  triage_status        text not null,        -- idle | pending | completed | expired
  triage_attempt_id    uuid null,
  last_triaged_event_ids jsonb not null default '[]', -- event ids absorbed at complete, not max(createdAt)
...
| `source` | Behavior |
|---|---|
| `startup` | Insert the row (`activation_epoch = 1`). Request `mode=full` on `GET /api/v1/briefing` — the shipped hook omits `mode` and gets the `brief` slice (`cmd/3ngram-hook/briefing.go`). Stamp `briefing_delivered_at` and `briefed_memories` (id, topic, status — not bare UUIDs) after delivery. |
...
SessionEnd is best-effort. Stop (and SessionStart) refresh `last_seen_at`. A row is
...
insert a new row. If the lease has expired but `closed_at` is still null, treat that
write as implicit close **then** reopen, and attach provenance, in one transaction —
the same post-idle write must not depend on whether the sweeper has run. Cancel a
closer job still queued for that run. If the closer already completed, leave those
memories; a later explicit close re-runs the closer only when there is untriaged
signal since. An **explicit** SessionEnd close does not resurrect on write — those
writes succeed unattributed (below). The closer on implicit close waits a grace after
lease expiry so an overnight idle gap can reopen instead of being debriefed
mid-conversation.
...
`last_message_excerpt` **only after the closer has durably consumed it** (or a TTL
sweep if the closer never runs). Explicit close must not clear it first — that is
the closer's conversation-bearing input.
...
ceiling — the closer then treats the run as incomplete for completion/watermark
purposes rather than pretending it saw everything. `jsonb` operators for
`sessionRunId` only, parsed through `sessionProvenancePayloadSchema`, never
`payload` as a blob. History DTO stays metadata-only (`present` / `jsonType` /
`byteLength`).
...
Stop(stop_hook_active=true)  OR  numeric self-cap hit
  → list event ids for this sessionRunId since attempt begin (uuidv7 order)
  → if that set is empty: triage_status=expired (closer may still run)
  → if non-empty: triage_status=completed, last_triaged_event_ids = that set
```

Do **not** complete on `stop_hook_active=true` after a zero-write continuation —
that suppresses the closer. Complete only when provenance events exist since
attempt begin; otherwise expire.

Do **not** watermark with `max(createdAt)`. That is the same Read Committed
timestamp race this page rejects for briefing cursors. The watermark is the **set
of event ids** visible at complete (bounded like `listSessionEvents`). Re-arm is
an event id not in the set — including a write that committed after the listing.
If the listing is truncated, expire rather than complete.
...
create / supersede / resolve / **unresolve** / **archive** whose id is not in
`last_triaged_event_ids`. That write **atomically** sets `triage_status` back to
`idle` (completed is not terminal). The closer also selects `completed` rows
whose event cursor has advanced, so a session that ends before the next Stop is
not skipped.
...
**Claim then write.** Before any `remember` / `resolve`, the job atomically claims
the row (`triage_status` → a processing state, or a `closer_attempt_id` unique per
run). Durable idempotency key = that attempt id. BullMQ retries (3) plus LLM
rephrasing would otherwise bypass content-hash dedup and append duplicate atoms.
Deleting the extras is forbidden (hard rule 1). If the process crashes after
claim and before completion, retry resumes the same attempt rather than a new
generation. Clear `last_message_excerpt` only after this attempt has durably
finished.
...
It runs on lease-expiry (after grace) or explicit `closed_at` when
`triage_status` is `idle`, `pending`, `expired`, **or `completed` with event ids
not in `last_triaged_event_ids`**. It does not scrape tool I/O. It does not run
on every Stop. It does not invent user-turn decisions that were never written.
...
The body carries `activation_epoch`. A close whose epoch does not match the
row's current epoch is a **no-op** — a delayed SessionEnd from the previous
activation must not close a resumed run.
...
| `POST` | `/api/v1/agent-sessions/:sessionRunId/close` | set `closed_at` if `activation_epoch` matches |
...
| 5 | REST open / close (`activation_epoch`) / heartbeat / prompt render. Hook SessionStart **and a heartbeat-only Stop** (no triage). Optional throttled PostToolUse heartbeat | Lease stays live across completed turns; resume/compact/crash still hold |
| 6 | Closer worker, same debrief rubric, atomic claim. **Worker image + Compose service** so self-host has a consumer | Commitment-recall **improvement** vs 0%, not merely ≥ the 26%/0% status quo; precision near zero-spurious |
