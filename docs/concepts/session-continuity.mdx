...
**Status: design, not built** (table, native `sessionRunId` plumbing, the typed
provenance read, and the session-lifecycle REST surface exist; hook / closer do
not). `agent_sessions` and the `memory_events` session expression index shipped in
...
ship the hook-facing half of step 5; the Go hook that calls them (step 5b) does
not exist yet, so nothing drives them in production. No triage routes, no Stop
nudge, no session-closer worker. Tracked as
