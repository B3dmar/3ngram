...
or open. The closer fences its claim, a re-check after the budget reservation and
immediately before the gateway call, and its final write-back on this epoch
(below), so an in-flight pass that
observed the pre-erasure epoch either never claims the row, is caught before it
sends the excerpt or briefed topics being redacted, or has its resolves and
bookkeeping rejected once it reaches its own epoch checks. **Residual, disclosed
rather than closed:** a pass that already dispatched to the gateway before the
erasure UPDATE commits has that request in flight, and no epoch bump recalls it —
the fence stops a pass from *landing* work on the redacted row, not from
*completing a round trip already under way*. That window is bounded by the
gateway's request timeout (30s default), so erasure can report complete while one
last request is still on the wire for up to that long.
...
4. **on the `resolve` write itself** (issue #185) — `transitionCommitment`
   carries the pass's fenced epoch as an EXISTS predicate on the SAME UPDATE
   that writes the commitment, evaluated by Postgres against the identical
   snapshot as the write. No separate statement, no gap for anything to land
   in between;
...
(3) is superseded by (4): the outer per-resolve read used to be the only fence
on an individual resolve, so **an individual resolve could commit against a
session that had just become active again** — that residual is now CLOSED, not
merely disclosed, by the write-time EXISTS predicate. (3) still earns its keep
as the cheap early exit that stops the REST of the batch without attempting a
write destined to fail. Resolve is reversible regardless (`unresolve`), which is
why v1 stayed resolve-only in the first place — a closer that could `remember`
would need a stronger guarantee than even (4) gives on non-idempotent writes,
and that is one of the costs the page rejects it for.
