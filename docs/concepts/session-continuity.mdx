...
The fence is checked in **four** places, and it is worth being precise about what
...
2. **after the budget reservation, immediately before the gateway call** (issue
   ...
   timeout;
3. **before each `resolve`** — a cheap read, so a resurrection during a slow
   generation stops the batch at the next candidate rather than at the end;
4. **on the final write-back**, together with the attempt token — the bookkeeping
   (terminal status, watermark, excerpt clear) is all-or-nothing.

(2) and (3) each narrow a window; neither closes it. Once (2) passes, the prompt
is dispatched, and nothing can recall a request already on the wire — the residual
is bounded by the gateway's timeout (30s default) and is disclosed, not hidden,
in the erasure section above. Once (3) passes for a given candidate, a
resurrection can still land between that check and the `resolve` call that
follows it, so **an individual resolve may commit against a session that has just
become active again**. Both are deliberate, disclosed trades rather than
oversights: the fence gates the BOOKKEEPING atomically, while individual resolves
are reversible. A commitment closed a moment too early is undone with `unresolve`
— which is the whole reason v1 is resolve-only. A closer that could `remember`
would need a real transaction boundary here instead, and that is one of the costs
the page rejects it for.
