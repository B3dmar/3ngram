...
- the pre-gateway check, immediately before dispatch — this one only NARROWS the
  ...
  erasure commit. **At most one request** may still dispatch racing that commit,
  and if it does, it is on the wire for up to the gateway's request timeout (30s
  default) before it completes — disclosed, not hidden, and not an absolute
  "≤ timeout" guarantee (see the fence enumeration below for why closing it fully
  would mean a lock across the network call, which the design rejects by
  contract);
...
actually starting) it is not provably zero. The honest bound: **at most one
request** may dispatch racing the erasure commit at that point, and if it does,
it is on the wire for up to the gateway's timeout (30s default) before it
completes — disclosed in the erasure section above, not an absolute "≤ timeout"
guarantee. Closing it fully would require holding a lock across the gateway
call — the design memo's option 2, rejected because it couples erasure latency
to the gateway's and inverts the repo's no-lock-across-network-call rule.
...
active again** — that residual is now CLOSED, not merely disclosed, by (4)'s
lock-then-read. (3) stays in place deliberately, as a cheap EARLY EXIT: it is a
plain read with no lock, so it cannot itself guarantee anything, but it stops
the REST of the batch at the first stale candidate rather than paying a doomed
lock-wait-then-write attempt for every remaining one. Resolve is reversible
regardless (`unresolve`), which is why v1 stayed resolve-only in the first
place — a closer that could `remember` would need a stronger guarantee than
even (4) gives on non-idempotent writes, and that is one of the costs the page
rejects it for.
