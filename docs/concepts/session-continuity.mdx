...
Import and `embed_failed` payloads stay on their own contracts. **Reject or
strip `sessionRunId` at the import boundary** (`packages/schema/src/import.ts`
accepts arbitrary bounded payload keys today). An imported lifecycle event that
