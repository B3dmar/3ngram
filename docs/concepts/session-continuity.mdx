...
Stop(stop_hook_active=true)  OR  numeric self-cap hit
  ...
  → if visible truncated: triage_status=expired
...
attempt N+1 when the set is replaced). Union across attempts; truncated → expire.

Do **not** watermark with `max(createdAt)`. That is the same Read Committed
timestamp race this page rejects for briefing cursors. Re-arm is an event id not
in `last_triaged_event_ids`, including a write that committed after the listing.
