...
**Status: design, not built** (table, native `sessionRunId` plumbing, and the
typed provenance read exist; hook / closer do not).
...
and stamp `{ sessionRunId }` on the audit events those writes emit, and
`GET /api/v1/agent-sessions/{sessionRunId}/events` lists them. No Stop nudge, no
session-closer worker. Tracked as
