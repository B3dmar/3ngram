...
**Status: design, not built** for the remaining step — the Stop nudge (step 7)
does not exist. Everything else on this page ships. `agent_sessions` and the
...
measured, and there are still no triage routes and no Stop nudge. Tracked as
...
`false`): with the flag unset the sweep scheduler is never registered, so nothing
implicitly closes a row and no generation is billed. Turning it on is the
measured decision the validation bar below governs, not a config convenience.
`apps/worker/Dockerfile` and the `worker` Compose service ship with it.
...
no-op — and the epoch is re-checked on the final write-back too, so an in-flight
generation whose epoch moved abandons rather than landing on a resumed session.
...
- **Debounce thresholds (step 7).** Tunable; pick defaults at implementation and
  pin them with tests (resume, compact, concurrent windows, failed close,
