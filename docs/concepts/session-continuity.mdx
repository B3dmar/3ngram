...
**Status: design, not built** no longer describes any STEP of the plan below —
every one of them, step 7b included, is now implemented. The marker stays
because what this page argues for is a *default* behavior, and that does not
exist: the Stop nudge ships **default-off** behind `THREENGRAM_STOP_NUDGE=1`
(`cmd/3ngram-hook/nudge.go`), registered for one harness, and the closer ships
default-off behind `SESSION_CLOSER_ENABLED`. Nothing on this page runs for a
user who has not opted in, and the **validation bar below has not been
measured**. Read it as built-but-unvalidated design until it has.
...
- **The numeric self-cap is the server's `pending` state**, not a counter. Stop
  ...
  absorbed them). The honest residual bound is **one nudge per turn that
  produced new provenance** — not "never twice in a session".
