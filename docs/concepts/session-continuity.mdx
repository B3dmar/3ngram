...
and, under the age guard below, a turn shorter than the floor defers even that to
the next Stop (the server cannot tell this caller from a sibling racing the same
Stop; both send the same natural key).
...
at most one Stop — a `pending` row can never arm a second injection, age only
grows, and `pending` is unconditionally closer-eligible if the session ends first.
