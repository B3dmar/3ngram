...
agent_sessions (
  ...
  last_message_excerpt text null,            -- bounded assistant text; closer input only; cleared when consumed or on explicit close
...
unhooked attribution default see "many open" and attach nothing forever. The lease
must outlast a plausible idle-open-terminal gap (overnight); the exact duration is
tunable.
...
clear `closed_at`, refresh `last_seen_at`, do not restamp briefing fields, do not
insert a new row. Cancel a closer job still queued for that run. If the closer already
completed, leave those memories; a later explicit close re-runs the closer only when
there is untriaged signal since. An **explicit** SessionEnd close does not resurrect
on write — those writes succeed unattributed (below). The closer on implicit close
waits a grace after lease expiry so an overnight idle gap can reopen instead of being
debriefed mid-conversation.
...
It must not open a session and must not restamp briefing fields. Null
`last_message_excerpt` once the closer consumes it, and on explicit close if it is
still set — it is assistant content at rest, not a memory.
...
1. The caller may pass only an opaque `sessionRunId`. The server resolves it to a
   tenant-owned `agent_sessions` row and **derives** `agent` / `project` from that
   row. A client-supplied agent or project on the write is rejected. **The memory
   write and the attachment are not the same decision.** Another tenant's run id
   (or a syntactically valid id that is not ours) **fails the write** — that is a
   cross-tenant probe. A closed or stale-lease row of *this* tenant **does not**:
   the `remember` / `revise` / `resolve` succeeds with payload unset. Bookkeeping
   going stale must not destroy curated writes. An implicitly-closed row also
   resurrects (see Lease); an explicitly-closed row does not.
...
3. **Hooked path always sends `sessionRunId`** (the hook opened the row at
   SessionStart and the MCP/SDK adapter the hook owns can inject the id). Step 5
   registers a **heartbeat-only** Stop so the lease stays fresh during active turns
   before the nudge exists. Overnight idle with no Stop still expires the lease;
   the next write or heartbeat resurrects rather than 400ing.
...
```text
Stop(stop_hook_active=false, triage_status=idle|expired)
...
`stop_hook_active=true` never injects a new prompt; it only finalizes. Claude's
eight-block override is configurable (`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`) and
`stop_hook_active` may never arrive (#54360), so the **numeric self-cap is a
...
**finalizes** that attempt (same watermark rule) and does not inject again. A live
session keeps refreshing the lease, so "expire pending after the lease" cannot fire
while the user is working — that expiry is only for a dead session the closer will
pick up. Interrupted triage in a live session is the next Stop, not the lease.
...
| 5 | REST open / close / heartbeat / prompt render. Hook SessionStart **and a heartbeat-only Stop** (no triage) | Lease stays live across active turns; resume/compact/crash still hold |
| 6 | Closer worker, same debrief rubric | Recall **and** precision vs the 26% / 0% / 0-spurious baseline |
| 7 | Gated Stop nudge, one project, one harness, default-off | Handshake absorbs its own writes; debounce; numeric cap (finalize path included) |
