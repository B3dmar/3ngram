...
**Status: design, not built** (table exists; write path / hook / closer do not).
`agent_sessions` and the `memory_events` session expression index shipped in
migration 0032. No write-time `sessionRunId` on native remember, no typed
...
close does not resurrect on write — those writes succeed unattributed (below). The
...
4. **Single-open-session default is the floor** (vanilla MCP and hooked sessions
   ...
   Serialize that decision against concurrent opens (transaction `FOR SHARE` on
   the candidate row, or a tenant advisory lock) so "unattributed rather than
   guessed wrong" still holds when two sessions open in the same moment — the
   same Read Committed genus as the watermark race.
...
**Claim then write, fenced by epoch.** Before any model call, the job atomically
claims the row at the current `activation_epoch` (`closer_attempt_id` unique per
run+epoch). If resurrection has incremented the epoch, the claim fails and the
job is a no-op — including an in-flight generation that re-checks the epoch
before each write. One rule; no separate cancel protocol.

**Per-output idempotency.** One attempt id cannot tell which atoms of a
partially-committed batch survived a crash, and hard rule 1 forbids deleting the
duplicates. Persist the generated batch under the attempt *before* any
`remember` / `resolve`, then write each atom with key `(attempt_id, ordinal)`.
Retries **replay** those rows; they never regenerate. BullMQ's three attempts
plus LLM rephrasing would otherwise bypass content-hash dedup.

Clear `last_message_excerpt` only after this attempt has durably finished.
...
`POST /api/v1/agent-sessions/close` sets `closed_at`. Addressed by natural key
plus `activation_epoch`. Idempotent. Fits the SessionEnd budget. Correctness does
not depend on it (the lease does). A close whose epoch does not match the row's
current epoch is a **no-op** — a delayed SessionEnd from the previous activation
must not close a resumed run.
...
| `POST` | `/api/v1/agent-sessions/close` | set `closed_at` if `activation_epoch` matches |
...
**Contract review is converged** (2026-08-21). Remaining discoveries belong in
step-level implementation review against this page, not more prose here.
