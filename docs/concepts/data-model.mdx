...
Shipped baseline: **24 Drizzle-modeled tables** (identity 10 · memory domain 8 · ops 3 · budget 3). SQL also contains leftover `subscriptions` / `stripe_events` from the platform split; they are filtered out of the public Drizzle schema (`drizzle.config.ts` `tablesFilter`) and are not part of this inventory. The scope guard is the physical count — every table beyond 24 needs a one-line justification in the PR description referencing a JTBD.
...
| `user_profile_attributes` | Display name and similar profile fields | RLS |
