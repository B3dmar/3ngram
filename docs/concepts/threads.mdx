...
Two new tables (17 → 19; the JTBD above is the budget justification — cross-scope, many-to-many thematic grouping is unrepresentable in the v1 layout):
...
One new tool — **`configure_thread`** — taking the MCP count to 11 of the hard cap of 12. It mirrors `configure_scope` (`list` / `create` / `rename` / `set_description` / `archive` / `delete`) and adds the curation actions (`pin` / `unpin`).
