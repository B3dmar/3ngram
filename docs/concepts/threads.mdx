...
**Status: design, not built.** No `threads` / `memory_threads` tables, no `configure_thread` tool, no `thread` filter. Tracked as [issue #109](https://github.com/B3dmar/3ngram/issues/109). Mixed shipped/design pages use their own status line (see [MCP surface](/concepts/mcp-surface)).
