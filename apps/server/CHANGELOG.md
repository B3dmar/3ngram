...
- a96fb41: Accept the ephemeral loopback port a native MCP client binds at `/oauth/authorize` (RFC 8252 §7.3), which unblocks Claude Code.
  ...
  A registered `redirect_uri` must now be an RFC 3986 URI string: any character outside printable ASCII, plus the printables the grammar excludes (`"`, `<`, `>`, `\`, `^`, `` ` ``, `{`, `|`, `}`), is rejected at the schema boundary and must be percent-encoded — `http://localhost/caf%C3%A9`, not `http://localhost/café`. RFC 6749 §3.1.2 already requires an absolute RFC 3986 URI, and both DCR and CIMD compose `redirectUriSchema`, so the rule lands on every registration path at once. It exists because the port matcher reads the raw path and query while the WHATWG parser reads the same bytes differently — `http://localhost\@evil.com/callback` MEANS `/@evil.com/callback` — and a character policy split across two layers would have relaxed the port for some accepted URIs and not others. `hasForbiddenUriCharacter` is exported so the port matcher can hold BOTH sides of a comparison to the same rule: the presented `redirect_uri`, which `authorizeRequestSchema` deliberately carries as an opaque string, and a row registered before this refine existed. A legacy row carrying such a character keeps byte-exact matching — exactly what it had before this release — instead of gaining a relaxation whose key could disagree with the parser. The constraint is still defined in one place; core imports it.
...
- 7e254c8: Refuse an agent-session open on an account that was erased while the request was in flight.
