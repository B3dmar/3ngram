...
// Node-side opaque encode/decode for the dashboard search frozen-ordering cursor.
...
// stays dependency-light (zod only — no @types/node for Buffer). The browser
// treats the token as opaque and never decodes it (see lib/search/api.ts).
 ...
 * mapRestError turns into a 400 — a forged or garbled cursor is client input,
