...
1. **Never merge/delete memory data on a write path.** Append-and-supersede only (docs/concepts/memory-model.mdx). Any code that destroys memory rows is a bug by definition. One carve-out, decided with issue #233: filing metadata (`scope`, `project`, `tags`) may change in place through the `revise` tool's move disposition, audited by a `revise` memory event that records the filing before and after; content, topic, status and the bi-temporal columns never change in place, and an `asOf` read therefore reports a moved memory under its current filing. — *Enforcement: **review**, plus the supersession suites in `packages/db` and the eval's `supersession_correct` floor.*
